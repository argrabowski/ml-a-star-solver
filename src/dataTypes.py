...
class BoardState(PrintableFields):
    ...
    def heuristic(self, board: list[list[Union[int, str]]], heuristicType: int) -> int:
        ...
        match heuristicType:
            ...
            case 7: 
                ans = self.MLHueristic(goalCoords)
    ...
    def MLHueristic(self, goal: Vector2) -> int:
        robotX = self.coords.x
        robotY = self.coords.y
        distX = abs(goal.x - robotX)
        distY = abs(goal.y - robotY)
        manDist = distX + distY
        diagDist = math.sqrt(math.pow(distX, 2) + math.pow(distY, 2))

        # Constant linear regression
        return (-0.0133 * robotX) + (-0.0029 * robotY) + (0.9954 * distX) + (0.987 * distY) + (0.6788 * manDist) + (1.2679 * diagDist) + 2.8424
